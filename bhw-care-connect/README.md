...
built from the Claude Design handoff in `design/BHW Care Connect Just Ask.dc.html`.
...
```
.                    the site — this is what builds and ships
├── src/
├── index.html
└── design/          the Claude Design handoff, kept for reference only
    ├── *.dc.html    the approved designs
    ├── tokens/      the source of truth for colour, type and spacing
    ├── assets/      brand marks at original resolution
    ├── chats/       the conversations the design came out of
    └── CLAUDE.md    the design system's own rules — read before restyling
```

Nothing in `design/` is imported by the app; it's the reference you check
against when something looks off.
